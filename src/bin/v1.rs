...
impl TouchIoctlProtocol for ProtocolV1 {
    fn set(&self, fd: i32, _touch_id: u32, mode: u16, value: i32) -> Result<(), String> {
        v1_ioctl(fd, REQ_SET_CUR, mode, &[value]).map(|_| ())
    }

    fn get(&self, fd: i32, _touch_id: u32, mode: u16, cmd: Cmd) -> Result<i32, String> {
        let buf = v1_ioctl(fd, cmd.req(), mode, &[])?;
        Ok(gi32(&buf, 0))
    }

    fn get_mode_all(&self, fd: i32, _touch_id: u32, mode: u16) -> Result<[i32; 6], String> {
    ...
    fn reset(&self, fd: i32, _touch_id: u32, mode: u16) -> Result<(), String> {
        v1_ioctl(fd, REQ_RESET, mode, &[]).map(|_| ())
    }

    fn set_long(&self, fd: i32, _touch_id: u32, mode: u16, values: &[i32]) -> Result<(), String> {
    ...
    fn print_specific_ioctl_codes(&self) {
        println!("=== V1 (sm8250 era — buffer layout: [mode | value/count | values...]) ===");
